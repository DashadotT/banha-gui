/*
   ============================================================
   BANHA LoRa Node 2 - v14 NON-BLOCKING SUPABASE QUEUE
...
#define LED_PIN 2
unsigned long ledIndicatorUntil = 0;
...
const unsigned long UPLOAD_RETRY_DELAY_MS = 500;
...
void supabaseDataUploaderTask(
  ...
  for (;;) {
    ...
    // Wait directly for the next item. This removes the old
    // 1-second polling interval from the successful path.
    if (
      xQueueReceive(
        dataUploadQueue,
        &item,
    ...
    // No post-upload delay. If another reading is queued,
    // upload it immediately; otherwise xQueueReceive() blocks.
  }
}
...
void loop() {
  ...
  // ----------------------------------------------------------
  // LoRa — highest-priority application path
  // ----------------------------------------------------------
  // Poll first so web/Wi-Fi work cannot add an avoidable delay
  // before receiving DATA or replying with an ACK.
  if (loraReady) {
    ...
    if (packetSize > 0) {
      ...
      // Non-blocking LED indication. Do NOT wait 50 ms here.
      digitalWrite(
        LED_PIN,
        HIGH);
  ...
  // ----------------------------------------------------------
  if (
    ledIndicatorUntil != 0 && (long)(millis() - ledIndicatorUntil) >= 0) {
  ...
  // ----------------------------------------------------------
  // Web server / Wi-Fi maintenance
  // ----------------------------------------------------------
  // These run AFTER the LoRa poll so radio traffic gets first
  // opportunity every loop iteration.
  server.handleClient();
  maintainWiFiConnection();

  // No delay() here. The application loop remains free-running.
}
