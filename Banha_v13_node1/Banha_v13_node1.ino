/*
  =====================================================
  BANHA_v13.ino
  NODE 1 - Node 2 heartbeat / connection status detection
...
const unsigned long NODE2_HEARTBEAT_INTERVAL_MS = 2000;
...
static void updateStatusLabels() {
  ...
  const char *nodeText = node2Connected ? "CONNECTED" : "DISCONNECTED";
  const lv_color_t nodeColor = node2Connected ? nodeConnectedColor : nodeDisconnectedColor;
...
void initLoRa() {
  ...
  */
  ...
  lastHeartbeatMillis = millis() - NODE2_HEARTBEAT_INTERVAL_MS;
...
// NODE 2 HEARTBEAT
//
// Node 1 periodically sends a PING when there is no other
// LoRa transmission in progress. Node 2 must reply using
// the existing ACK format:
//   NODE:2,TYPE:ACK,SEQ:<same>,ACKTYPE:PING
//
// This makes the UI status represent actual Node 1 <-> Node 2
// communication rather than merely local LoRa initialization.
// =====================================================
...
void manageNode2Heartbeat() {
  ...
  if (millis() - lastHeartbeatMillis < NODE2_HEARTBEAT_INTERVAL_MS) return;
...
void noiseSamplingTask(void *parameter) {
  ...
  while (noiseTaskRunning) {
    ...
    // I2S data. The main loop remains free for LVGL, touch, LoRa and SCD41.
...
void loop() {
  ...
  // ---------------------------------------------------
  // Actual Node 1 <-> Node 2 connection check. This only
  // reports CONNECTED after Node 2 has actually replied.
  // ---------------------------------------------------
