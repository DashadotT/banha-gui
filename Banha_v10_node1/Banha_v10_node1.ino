/*
  ...
  =====================================================
  1. RELIABLE LoRa COMMANDS / DATA
     - Every outgoing message (START, STOP, DATA) now
       carries a unique, ever-increasing sequence number:
         NODE:1,TYPE:START,SEQ:12
         NODE:1,TYPE:STOP,SEQ:13
         NODE:1,TYPE:DATA,PACKET:3,SEQ:14,AVG_CO2:...,AVG_TEMP:...,AVG_NOISE:...
     - After sending, Node 1 switches the radio back to
       listen mode and waits (briefly, without blocking
       LVGL/touch/SCD41) for an ACK from Node 2:
         NODE:2,TYPE:ACK,SEQ:14,ACKTYPE:DATA
     - If no ACK arrives before the timeout, the message
       is automatically retransmitted (same SEQ), up to
       TX_MAX_ATTEMPTS times.
     - Commands (START/STOP) and DATA packets are queued
       separately, with commands sent first, so a button
       press is not stuck behind a slow DATA retry.
     - This whole state machine is NON-BLOCKING: it is
       driven from loop() a little bit every iteration,
       so the touchscreen, LVGL redraw, and SCD41 polling
       keep running normally while a retry is in flight.

  2. NOISE SENSOR - CONTINUOUS 5-SECOND AVERAGING
     - The INMP441 is now sampled a little bit on every
       pass through loop() (a short, non-blocking I2S
       read), and the running sum-of-squares is
       accumulated for the whole 5-second window.
     - At the end of each 5-second window the RMS for the
       ENTIRE window is converted into an estimated dB
       value (with light smoothing), instead of a single
       ~100ms snapshot taken only at the 5-second mark.

  3. NOISE CALIBRATION
     - The INMP441 gives a relative sound-pressure
       reading, not a calibrated dBA value. The constants
       below (NOISE_REF_RMS / NOISE_REF_DB) are a starting
       point tuned for "quiet classroom" style monitoring.
       See the calibration note near those constants for
       how to tune them for your actual room/hardware.
     - All noise values reported by this system remain
       ESTIMATED sound levels, not certified dBA
       measurements.

  Everything else (LVGL UI, touch, SCD41, TFT, LoRa pins,
  general architecture) is unchanged from v8.
  =====================================================
...
// =====================================================
// SEND START / STOP COMMANDS (NOW QUEUED + RELIABLE)
// =====================================================
...
// =====================================================
// CALCULATE AND SEND AVERAGE (NOW QUEUED + RELIABLE)
// =====================================================
